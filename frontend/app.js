...
function setBusy(busy) {
  store.busy = busy;
  ui.sendButton.disabled = busy || !store.sessionId || store.turn >= 10;
  ui.messageInput.disabled = busy || store.turn >= 10;
  ui.newSessionButton.disabled = busy;
  ui.processingStatus.textContent = busy ? "Agent processing…" : "";
  ui.pipeline.classList.toggle("processing", busy);
  if (busy) ui.pipeline.classList.remove("complete");
}
...
function welcomeMarkup() {
  return `
    <div class="welcome" id="welcome">
      ...
      <p>Describe what you need. The agent will refine your intent across turns.</p>
      <div class="example-prompts" id="examplePrompts">
        <button type="button">Running shoes under $100</button>
        <button type="button">A black jacket for everyday wear</button>
        <button type="button">Comfortable shoes for walking</button>
        <button type="button">I'm looking for something but I'm still exploring</button>
      </div>
    </div>`;
}

function bindExamples() {
  document.querySelectorAll("#examplePrompts button").forEach((button) => {
    button.addEventListener("click", () => {
      ui.messageInput.value = button.textContent.trim();
...
function resetDemoPanels() {
  ...
  ui.conversation.innerHTML = `<div class="welcome"><div class="welcome-icon">▶</div><h3>Evaluator scenario ready</h3><p>Click Next Turn or Auto Run. User messages are generated by the official evaluator flow.</p></div>`;
...
async function newSession() {
  ...
  try {
    ...
    store.turn = payload.turn;
    store.developer = { agent_response: null, state: null };
    ui.sessionId.textContent = store.sessionId;
    ui.sessionId.title = store.sessionId;
    ui.turnCount.textContent = "0";
    ui.conversation.innerHTML = welcomeMarkup();
    ui.recommendations.innerHTML = `
      <div class="empty-state">
        <div class="empty-grid" aria-hidden="true"><span></span><span></span><span></span></div>
        <h3>Real catalog results appear here</h3>
        <p>Recommendations are retrieved and reranked by the existing Shopping Copilot pipeline.</p>
      </div>`;
    ui.resultCount.textContent = "Awaiting your brief";
    ui.stateContent.innerHTML = `
      <div class="state-placeholder">
        <div class="radar" aria-hidden="true"><span></span></div>
        <p>State will update as the agent learns your preferences.</p>
      </div>`;
    ui.developerJson.textContent = "No response yet.";
    ...
    showError(error.message || "Unable to start a new session.");
  } finally {
    setBusy(false);
  }
}
...
async function sendMessage(message) {
  ...
  if (!cleanMessage || store.busy) return;
  if (!store.sessionId) {
    showError("Session is still starting. Please try again.");
    return;
  }
  if (store.turn >= 10) {
    showError("This session has reached 10 turns. Start a new session to continue.");
    return;
  }
  ...
  try {
    const payload = await api("/api/chat", {
      method: "POST",
      body: JSON.stringify({ session_id: store.sessionId, message: cleanMessage }),
    });
    store.turn = payload.turn;
    store.developer = payload.developer || { agent_response: null, state: payload.state };
    ui.turnCount.textContent = String(payload.turn);
    ...
    renderDeveloperData();
    ui.pipeline.classList.add("complete");
    if (store.turn >= 10) showError("Turn 10 reached. Start a new session for another demo.");
  } catch (error) {
    appendMessage("agent", "I couldn't complete that request. Please try again or start a new session.");
    showError(error.message || "Network error. Please try again.");
  } finally {
    setBusy(false);
    if (store.turn < 10) ui.messageInput.focus();
  }
}
