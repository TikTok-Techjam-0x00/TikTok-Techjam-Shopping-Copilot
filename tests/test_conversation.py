...
class ConversationStateTest(unittest.TestCase):
    def test_accumulates_and_overrides_category_without_losing_budget(self) -> None:
        ...
        self.assertEqual(state.hard_constraints["budget"], 120.0)
        self.assertEqual(state.hard_constraints["use_case"], "hiking")
    ...
    def test_direct_answer_uses_previous_asked_attribute(self) -> None:
        ...
        self.assertEqual(state.hard_constraints["use_case"], "winter")

    def test_no_preference_rejects_and_removes_slot(self) -> None:
        state = create_state()
        state.add_hard_constraint("color", "blue")
        ...
        self.assertIn("color", state.rejected_attributes)
    ...
    def test_pipeline_contract_is_json_serializable(self) -> None:
        state = create_state({"preference_tags": ["fit"]})
        update_state(state, "Find me black running shoes under $100", turn=1)

        payload = state.to_dict()

        json.dumps(payload)
        self.assertEqual(payload["known_attributes"]["category"], "black running shoes")
        self.assertEqual(payload["known_attributes"]["budget"], 100.0)
        self.assertEqual(payload["asked_attributes"], [])

    def test_retrieval_text_never_contains_cjk_content(self) -> None:
        state = create_state()
        state.category = "running shoes " + "\u767b\u5c71\u978b"
        state.soft_preferences.append("waterproof " + "\u9632\u6c34")
